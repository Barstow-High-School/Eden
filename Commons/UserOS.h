...
namespace UsOS {
	...
	*/
	...
	bool Preprocess() {
		float min = 0.5;
		in temp;
		temp.Accelerometer.Calibrate();
		temp.Gyroscope.Calibrate();
		if (temp.Accelerometer.Magnitude() > min || temp.Gyroscope.Magnitude() > min || temp.Battery.Critical()) {
			Preprocess();
		}
		if (bknd.LMM > bknd.RMM) {
			bknd.WMM = 1 / bknd.LMM;
		}
		else {
			bknd.WMM = 1 / bknd.RMM;
		...
		bknd.RMM = bknd.RMM * bknd.WMM;
		bknd.LMM = bknd.LMM * bknd.WMM;

		if (bknd.LMM > bknd.RMM) {
			bknd.timemult = (1 - bknd.RMM) + 1;
		}
		else {
			bknd.timemult = (1 - bknd.LMM) + 1;
		}
